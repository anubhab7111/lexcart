...
- **Webhook replay.** `payment.captured` and `payment_link.paid` are idempotent by design (`confirm_payment`/`confirm_campaign_link_payment` check existing state before acting), but there's no explicit dedup-by-event-id table — a replayed *valid* signed event for an already-settled order is a no-op today because the order is no longer `created`, not because the event id was recognized and dropped. Fine at this scale; a production system would still track processed webhook event ids.
...
- **Mock-gateway everything.** Every transcript in `EVALUATION.md` runs on the mock Razorpay gateway (no keys configured) — the code paths for real test-mode keys (`razorpay_gateway.py`'s non-mock branches, `sign_webhook_payload`/`verify_webhook_signature` against Razorpay's real webhook secret format) are written and exercise the same interfaces, but haven't been independently verified against live Razorpay test-mode traffic in this submission.
- **Auto-refund safety net is defensive, not harness-tested.** If booking creation fails *after* a payment is captured, `confirm_payment` auto-refunds and marks the order `refunded` rather than leaving a paid-but-unfulfilled order — this path exists in the code (`server/app/commerce/orders.py`) but isn't independently triggerable through the public API surface the harness drives, so it's asserted by code review, not by an automated test.
- **Merchant role gating is opt-in.** `MERCHANT_EMAILS` defaults to empty, which means *any* signed-in user can approve campaigns and see the audit trail in this demo configuration — intentional for a frictionless buildathon demo, but the allowlist must be set before this could be a real merchant tool.
